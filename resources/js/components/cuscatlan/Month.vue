<template>
  <div data-app>
    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Meses</v-toolbar-title>
          <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    async initialize() {
      ...
      const res = await monthApi.get().catch((error) => {
      ...
      this.records = res.data.months;
      this.recordsFiltered = res.data.months;
      this.loading = false;
    },
