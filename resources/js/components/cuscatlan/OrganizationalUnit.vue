<template>
  <div data-app>
    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Unidades organizativas</v-toolbar-title>
          <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    async initialize() {
      ...
      let requests = [organizationalUnitApi.get(), directionApi.get()];
      ...
      this.records = responses[0].data.organizationalUnits;
    ...
    editItem(item) {
      ...
      this.$v.editedItem.direction_name.$model = this.editedItem.direction_name;
    },
    ...
    async deleteItemConfirm() {
      const res = await organizationalUnitApi
    ...
    async save() {
      this.$v.$touch();
      if (this.$v.$invalid || this.editedItem.direction_name == "") {
    ...
    searchValue() {
      ...
      if (this.search != "") {
        this.records.forEach((record) => {
          let searchConcat = "";
          for (let i = 0; i < record.ou_name.length; i++) {
            searchConcat += record.ou_name[i].toUpperCase();
            if (
