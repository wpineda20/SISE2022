<template>
  <div data-app>
    ...
    <v-data-table
      ...
      sort-by="financing_name"
      class="elevation-3 shadow p-3 mt-3"
    >
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Financiamientos</v-toolbar-title>
          <v-spacer></v-spacer>
                  ...
                  <v-row>
                    ...
                    <v-col cols="12" sm="12" md="12">
                      <base-input
                        ...
                        v-model="$v.editedItem.financing_name.$model"
...
export default {
  data: () => ({
    ...
    dialogDelete: false,
    headers: [
      { text: "NOMBRE", value: "financing_name" },
  ...
  methods: {
    async initialize() {
      ...
      const res = await financingApi.get().catch((error) => {
      ...
      this.records = res.data.financings;
      this.recordsFiltered = res.data.financings;
    },
    ...
    searchValue() {
      ...
      if (this.search != "") {
        this.records.forEach((record) => {
          let searchConcat = "";
          for (let i = 0; i < record.financing_name.length; i++) {
            searchConcat += record.financing_name[i].toUpperCase();
            if (
